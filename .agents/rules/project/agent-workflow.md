...
- [MUST] When a subagent runtime exists, skills that implement, review, verify, or apply authorized review fixes follow `docs/agents/orchestration.md`: **hard try** pack roles `Executor` / `Reviewer` / `Verifier` (bodies in `agents/`), then fall back to the host’s general worker or the parent. [MUST NOT] shadow host built-in explore/plan/general names. [MUST NOT] fail a skill because a pack subagent is missing or spawn fails.
- [MUST] Authorized post-review fixes (“fix these” / implement in-scope auto-fix) go through the same `Executor` ladder when that role is loadable, not only ad-hoc parent edits.
