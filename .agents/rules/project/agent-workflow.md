...
- [SHOULD] Keep `AGENTS.md` as wiring + Mistakes only; put durable detail in `.agents/rules/**` and docs. Prefer empty-rewrite over accretion; prune Mistakes the current model no longer trips on.
...
- [SHOULD] Prefer the **LIGHT** track by default: bounded → `/implement`; multi-slice → `/to-spec` → `/to-tickets` → `/implement`; same-session pin → `/grilling`; mild coding-adjacent intent gap → `/product-fog`; multi-session eng fog → `/wayfinder` (prefer grill if one session is enough).
...
- [MUST] When the Agent tool exists, skills that implement, review, verify, or apply authorized review fixes follow `docs/agents/orchestration.md`: **hard try** project agents `Executor` / `Reviewer` / `Verifier` (Capitalized), then fall back to built-in `general-purpose` or the parent. Use built-in `Explore` / `Plan` for research — [MUST NOT] override those built-in names. [MUST NOT] fail a skill because a project subagent is missing or spawn fails.
- [MUST] Authorized post-review fixes (“fix these” / implement in-scope auto-fix) go through the same `Executor` ladder, not only ad-hoc parent edits when `Executor` is available.
