...
- Subagents (portable): `agents/` — `Executor`, `Reviewer`, `Verifier` (role briefs; host adapters optional). Skills **must try** the mapped role when a subagent runtime exists, then fall back to host general worker / parent; never hard-fail if missing. Details: `agents/README.md`, `docs/agents/orchestration.md`
