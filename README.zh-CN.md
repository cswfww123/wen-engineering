...
- `/grill-me` stress-test **工程** plan（同会话钉住）；加载 `/grilling`，并与 `/domain-modeling` 同时生效。
- `/grilling` 可复用的一次一问访谈循环（facts vs decisions、确认门）。
...
- `/wayfinder` 将跨会话雾清成 decision ticket map，直到可写诚实 coding spec。
...
- [`grill-me`](skills/grill-me/SKILL.md) - 用户调用的同会话 plan pin（LIGHT G）；加载 `/grilling`，并与 `/domain-modeling` 同时生效。
- [`grilling`](skills/grilling/SKILL.md) - model-invoked 访谈循环：一次一问、facts vs decisions、确认门。
- [`product-fog`](skills/product-fog/SKILL.md) - LIGHT 意图钉：编码邻域迷你 docket + 一条下一跳（非完整 PM）。
- [`wayfinder`](skills/wayfinder/SKILL.md) - 多会话 decision ticket map，直到可写诚实 coding spec。
