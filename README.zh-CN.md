...
- 如果技术栈或仓库形态已经确定，先运行 `/setup-project-harness`。它会先建立工作台：`AGENTS.md`、`CLAUDE.md`、`docs/agents/` 和本地 scratch 空间。
- 如果只有产品目标，价值/用户/outcome 仍开放（**HEAVY**），先走完整 PM（`wen-pm` `/pm-intake` 或团队流程）——禁止编造答案；settled 后再进 LIGHT 编码。
- 如果技术栈仍开放，做一轮很短的 `/grill-me`，再 `/setup-project-harness`。
...
- **G：** 同会话 `/grill-me` + `/domain-modeling`——LIGHT 一等工具；几条用户决策能一次问清时用；**不是** HEAVY PM 替代品。
- **L3：** 仅 coding 邻域钉意图（返工 / 轻度 Expected）。禁止编造 Expected。常路由到 **G**。
- **L4：** 产品已够 settled，技术路线仍跨会话雾。优先 G；地图要薄（首发 ≤5 票、research 优先）；结案后直接 L2。短提示词见 `skills/wayfinder/CONTINUE.md`。
...
inputs，也不会为了新术语被原地重命名。`/grill-me` 是用户调用的访谈入口；`/grilling`
是它（以及 Wayfinder HITL ticket）加载的共享 model-invoked 循环。同步会删除已
退役命令的 managed copies；若 canonical skills 目录中存在未标记的同名 skill，
普通同步会安全阻断，`--force` 则会先备份到 active skills root 之外再移除。
...
- `/grill-me` stress-test **工程** plan（同会话钉住）；加载 `/grilling`，并与 `/domain-modeling` 同时生效；收口前过 close gate（决策归档、snippet 规则扫描、高风险确认）。
- `/grilling` 可复用访谈循环：默认 **决策面 / batch 表**（非串行微问题），facts vs decisions、反橡皮图章、确认门。
...
- [`grill-me`](skills/grill-me/SKILL.md) - 用户调用的同会话 plan pin（LIGHT G）；加载 `/grilling`，MVP 边界 + close gate + 实现交接。
- [`grilling`](skills/grilling/SKILL.md) - model-invoked 访谈循环：决策面/batch 默认、facts vs decisions、反橡皮图章、确认门。
