...
inputs，也不会为了新术语被原地重命名。`/grill-me` 替代 `/grilling`（同一访谈协议）。
...
- `/grill-me` stress-test **工程** plan（同会话钉住）；与 `/domain-modeling` 同时生效。
...
- [`grill-me`](skills/grill-me/SKILL.md) - 一次一个问题的访谈协议，用于同会话 plan pin（LIGHT G）；与 `/domain-modeling` 同时生效。
