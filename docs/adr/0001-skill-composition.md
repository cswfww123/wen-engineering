...
The repo grew an implicit linear pipeline — `to-prd → to-issues → do-issues →
...
security at `security-review`, a bug fix at `diagnosing-bugs`. Cross-references are
also net-shaped, not groupable: `deep-code-trace` is referenced from 17 places and
...
4. **New skills fill the enterprise right-side gaps under this boundary:**
   `security-review` (system-level threat model and audit) and `ship` (release
   readiness plus version, notes, and rollback). Ops, observability, and data
   migration stay project-harness concerns, not general skills.

## Considered Options

- **Physical subdirectories per role.** Rejected: shared skills like
  `deep-code-trace` have no clean home, and reorganization breaks the sync
  manifest, README layout, relative references, and installed-user upgrades.
...
- Five existing skills (`to-prd`, `to-issues`, `do-issues`, `to-test-plan`,
  `qa-run`) lose their "Recommended Next Step" command chains, replaced by
  unordered `Related Skills` lists.
- `security-review` and `ship` declare what they do *not* do (diff review;
  build/deploy/sign) so they stay non-overlapping with `code-review` and CI.
