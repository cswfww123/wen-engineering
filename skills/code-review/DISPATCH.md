...
3. After candidates are collected, **must try** pack `Verifier` (or parent runs
   ...
   Incomplete production surface that survives verification blocks `Pass`.
...
5. Report **`agents used`** (which axes ran on Reviewer / host-general / parent)
   and **incomplete-surface**: `clean` | findings | `n/a`.
...
paths, dual-source domain facts, config stand-ins vs sibling real sources) is
blocking — not "intentional" merely because a comment says "later". Completion
claims fail while it remains.
...
Then: surviving findings; incomplete-surface clean|findings|n/a; rejected
groups (brief); verification gaps.
```
