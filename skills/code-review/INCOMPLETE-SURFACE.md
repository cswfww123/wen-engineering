...
non-ship), the classifier does **not** fire.

## Required behavior

| Role | Must |
| --- | --- |
| **Executor / `/implement`** | Finish the real domain step for claimed AC, **or** stop and report a blocker. Never land an incomplete surface and call the slice done. |
...
| **`/code-review` Correctness** | Incomplete surface on a production path is a **blocking** finding (confidence usually `100` when a comment admits deferral; `>=80` when dual-source or placeholder is proven). Verdict cannot be `Pass`. |
| **Verifier** | Completion claims fail when an incomplete surface remains for claimed AC. |
