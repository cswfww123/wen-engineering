...
- **Diagnosis only** (default) — “debug”, “diagnose”, “why failing”, or a named
  non-runnable `bug-report`: build the loop, reproduce, hypothesise, instrument.
  Prefer disposable harnesses in temp/scratch. No tracked production edits,
  permanent regression tests, or tracker mutation until fix is authorized.
- **Fix authorized** — only when the user explicitly asked for a fix or an
  active `/implement` (or equivalent) scope already authorizes code changes.
  Then run Phase 5–6. Record a review fixed point before production edits.
- A named `bug-report` stays diagnosis-only until a user-invoked workflow
  converts it.
...
**WEN:** skip this phase under diagnosis-only; return root cause + disposition
