...
6. **Frontier batch tables** — each round posts only the **frontier** (decisions with settled prerequisites). Preferred shape: one markdown table of **5–8** product/architecture decisions (max **10** without “继续深烤”). Recompute the frontier after my answers. Implementation minutiae → park for implement / tdd, do not serial-grill.
...
3. **`/code-review`** before commit (that skill picks **light** or **full**). Incomplete surface is blocking (sibling write paths, report filters, and historical values that still disagree with the settled target count as incomplete unless explicitly out of scope). Spec / Slice review must dual-read product doc and session AC — unauthorized PRD partial cannot Pass. UI Fidelity worker only when that skill marks it in scope (pin or restyle).
