...
issue. Run `gh` inside the clone so it infers the repository from the remote.
...
- List exhaustively: `gh api --paginate 'repos/{owner}/{repo}/issues?state=open&per_page=100'`
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues?state=all&per_page=100' --jq '.[] | select(.pull_request == null and ((.body // "") | test("(?m)^Kind: spec$"))) | {number,title,state,body,html_url}'
```
...
`--remove-blocking`. Do not assume an older installed client exposes them.

When flags are missing but the server exposes the REST capability, use:

```bash
gh api repos/{owner}/{repo}/issues/<child> --jq .id
gh api --method POST repos/{owner}/{repo}/issues/<parent>/sub_issues -F sub_issue_id=<child-rest-id>
gh api --method POST repos/{owner}/{repo}/issues/<ticket>/dependencies/blocked_by -F issue_id=<blocker-rest-id>
gh api --paginate repos/{owner}/{repo}/issues/<parent>/sub_issues
gh api --paginate repos/{owner}/{repo}/issues/<ticket>/dependencies/blocked_by
gh api --method DELETE repos/{owner}/{repo}/issues/<parent>/sub_issue -F sub_issue_id=<child-rest-id>
gh api --method DELETE repos/{owner}/{repo}/issues/<ticket>/dependencies/blocked_by/<blocker-rest-id>
```
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues?state=open&labels=ready-for-agent&per_page=100' --jq '.[] | select(.pull_request == null and ((.body // "") | test("(?m)^Kind: implementation-ticket$")) and ((.body // "") | test("(?m)^Mode: AFK$")) and (.assignees | length == 0)) | {number,title,body,labels,assignees,html_url}'
gh api --paginate 'repos/{owner}/{repo}/issues?state=open&labels=ready-for-human&per_page=100' --jq '.[] | select(.pull_request == null and ((.body // "") | test("(?m)^Kind: implementation-ticket$")) and ((.body // "") | test("(?m)^Mode: HITL$")) and (.assignees | length == 0)) | {number,title,body,labels,assignees,html_url}'
```
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues/<number>/dependencies/blocked_by?per_page=100' --jq '.[] | select(.state == "open") | .number'
```
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues/<map>/sub_issues?per_page=100' --jq '.[] | select(.state == "open" and ((.body // "") | test("(?m)^Kind: wayfinder-ticket$")) and (.assignees | length == 0))'
```
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues?state=open&per_page=100' --jq '.[] | select(.pull_request == null and ((.body // "") | test("(?m)^Kind: wayfinder-ticket$")) and ((.body // "") | test("(?m)^Parent: #<map>$")) and (.assignees | length == 0))'
```
...
```bash
comment_id="$(gh api --method POST repos/{owner}/{repo}/issues/<source>/comments -F body=@<qa-report> --jq .id)"
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues?state=open&labels=needs-triage&per_page=100' --jq '.[] | select(.pull_request == null and ((.body // "") | test("(?m)^Kind: bug-report$")))'
```
...
```bash
gh api --paginate 'repos/{owner}/{repo}/issues?state=all&per_page=100' --jq '.[] | select(.pull_request == null and any(((.body // "") | split("\n"))[]; . == "Origin: #<report>" or . == "Origin: <report-url>")) | {number,title,state,body,html_url}'
```
