...
Multi-file or non-trivial cleanup: **hard try** pack `Executor` per `docs/agents/orchestration.md` (then host general worker → parent). Never abort simplify if `Executor` is missing.
