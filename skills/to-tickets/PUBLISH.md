...
approved graph (coverage including PRD Inventory `SRC`→`Covers`, vertical
slices, acyclic blockers, frontiers, no invented Expected). Do not publish
while that gate fails. `Supports` is not coverage.
